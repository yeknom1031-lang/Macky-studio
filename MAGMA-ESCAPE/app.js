...
import {ANIMATION_NAMES,animatedArt,paintArt} from './animation.js';
...
function showBattle(){battle={time:0,hits:0,lives:3,cooldown:0,remaining:20,cursor:.5,ending:false,animation:'enemy-idle',animationTime:0};openDialog('battle',`<p class="section-kicker">GOLEM CHALLENGE / BONUS +40</p><h2>ゴーレムとぶつかった！</h2><div class="battle-arena">${animatedArt('enemy-idle','マグマゴーレム','battle-enemy',384)}<span class="battle-impact">HIT!</span></div><div class="enemy-health" aria-label="ゴーレムの体力"><i></i><i></i><i></i></div><div class="battle-lives" id="battle-lives">♥ ♥ ♥</div><div class="battle-score" id="battle-score">ヒット 0 / 3 ・ のこり20秒</div><div class="battle-track"><i class="target"></i><i id="battle-needle" class="needle"></i></div><p id="battle-message" class="battle-message">白いバーが緑のゾーンに来たらタップ！</p>${button('attack','タイミングアタック！')}<p class="small-note">3回成功で勝利。失敗3回で空腹 −15。<br>この間、マグマは止まっています。</p>`);}
function attack(){if(!battle||battle.cooldown>0||battle.ending)return;const ok=battle.cursor>=.35&&battle.cursor<=.65;battle.cooldown=.8;battle.animation=ok?'enemy-hit':'enemy-attack';battle.animationTime=0;const arena=$('.battle-arena');arena.classList.remove('hit','miss');void arena.offsetWidth;arena.classList.add(ok?'hit':'miss');if(ok){battle.hits++;sound.play('hit');$('.battle-impact').textContent=Math.abs(battle.cursor-.5)<.055?'PERFECT!':'HIT!';$('#battle-message').textContent='ナイス！ ゴーレムにヒット！';$$('.enemy-health i').forEach((e,i)=>e.classList.toggle('empty',i<battle.hits));}else{battle.lives--;sound.play('bad');$('#battle-message').textContent='惜しい！ 緑のゾーンを狙おう。';}if(battle.hits>=3||battle.lives<=0)battle.ending=true;}
...
function launchGacha(entries,preview=false){if(cinema)return;closeDialog();resetInput();$('#panel').inert=true;$('#home').inert=true;cinema=new GachaScene({host:$('#app'),entries,sound,assets,reduced:save.reduced||matchMedia('(prefers-reduced-motion: reduce)').matches,preview,onEquip:id=>{if(save.owned.includes(id)){save.equipped=id;persist();}},onFinish:()=>{cinema=null;$('#panel').inert=false;$('#home').inert=false;if(!preview){save.pending=null;persist();}panel(panelType==='collection'?'collection':'gacha');$('#panel .back-button').focus({preventScroll:true});}});}
...
function tick(now){
  ...
  if(!document.hidden&&!inactive){
    ...
    if(battle&&dialogType==='battle'){
      battle.time+=dt;battle.animationTime+=dt;battle.remaining-=dt;battle.cooldown=Math.max(0,battle.cooldown-dt);battle.cursor=.5+Math.sin(battle.time*3.8)*.47;
      if(battle.cooldown===0&&battle.animation!=='enemy-idle'){battle.animation='enemy-idle';battle.animationTime=0;}
      const actor=$('.battle-enemy');actor.dataset.animation=battle.animation;paintArt(actor,assets,battle.animationTime,{fps:battle.animation==='enemy-idle'?4:5,loop:battle.animation==='enemy-idle',reduced:save.reduced||motionPreference.matches});
...
async function init(){
  sync();const names=['world','world-mid','world-sky','hero','platforms','lobby','items','enemy','gacha','icon','capsule',...AURAS.map(a=>'aura-'+a.id),...ANIMATION_NAMES];let loaded=0;const failed=[];
